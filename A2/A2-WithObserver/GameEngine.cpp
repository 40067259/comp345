...
GameEngine::GameEngine() {
    phaseScreen = nullptr; //NEW
    statisticsScreen = nullptr; //NEW
}
...
void GameEngine::GameStart() {
    ...
    if (option == 1) {
        ...
        setObserver(true);
        phaseScreen = new PhaseObserver(this); //NEW
        statisticsScreen = new StatisticsObserver(this); //NEW
    }
    else {
...
void GameEngine::reinforcementPhase() {
    ...
    for (Player* p : playersVector) {
        ...
        setPhase("Reinforcement Phase for player" + p->getName());//NEW FOR OBSERVER
        notify();
...
void GameEngine::issuingOrderPhase()
{
    ...
    for (Player* p : playersVector)
    {
        setPhase("Issuing Order Phase for player" + p->getName());//NEW FOR OBSERVER
        notify();
...
void GameEngine::ordersExectionPhase()
{
    ...
    for (Player* p : playersVector)
    {
            ...
            setPhase("Execution Order Phase for player" + p->getName());//NEW FOR OBSERVER
            notify();
